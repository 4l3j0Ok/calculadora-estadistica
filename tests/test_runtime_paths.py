"""Tests de src/services/runtime_paths.py.
...
from src.services import runtime_paths


def test_linux_con_xdg_data_home(
...
def test_linux_sin_xdg_data_home(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(runtime_paths.sys, "platform", "linux")
    ...
    assert (
        db_path
        == tmp_path / ".local" / "share" / "calculadora-estadistica" / "history.db"
    )
    assert db_path.parent.is_dir()


def test_windows_con_localappdata(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_windows_sin_localappdata(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(runtime_paths.sys, "platform", "win32")
    monkeypatch.delenv("LOCALAPPDATA", raising=False)
    ...
    assert (
        db_path
        == tmp_path / "AppData" / "Local" / "calculadora-estadistica" / "history.db"
    )
    assert db_path.parent.is_dir()
