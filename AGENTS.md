...
- Fast syntax/import check: `uv run python -m compileall main.py controllers schemas services`.
...
- `main.py` is the app entrypoint. It creates `QApplication`, imports `resources_rc` for Qt resource registration, registers `calculadoraController`, `dispersionController`, `historyController`, `markdownController`, and `appVersion` in QML, then loads `qml/Main.qml` via `services.runtime_paths.app_base_dir()`.
- Keep QML slot/property names compatible with those context properties; the QML pages call Python slots directly.
- Parsing and user-facing Spanish validation errors are centralized in `services/parser.py`; statistical calculation is in `services/calculator.py` and `services/descriptive_stats.py`; controllers should mainly translate between QML JSON/models and services.
- Pydantic models live in `schemas/`; frequency, dispersion, and history schemas are separate even when parser helpers are shared.
- Markdown copy/paste uses `services/markdown_io.py` with Jinja2 templates from `assets/templates/*.md.j2`; those templates are embedded through `resources.qrc` but also have a filesystem fallback for tests.
...
- History is SQLite via `services/history_service.py` and must stay in the user data dir: `${XDG_DATA_HOME:-~/.local/share}/calculadora-estadistica/history.db` on Linux/macOS or `%LOCALAPPDATA%\calculadora-estadistica\history.db` on Windows.
